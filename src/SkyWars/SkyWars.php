...
class SkyWars extends PluginBase implements Listener {
    public $prefix = TextFormat::GRAY . "[" . TextFormat::WHITE . TextFormat::BOLD . "S" . TextFormat::RED . "W" . TextFormat::RESET . TextFormat::GRAY . "] ";
	public $mode = 0;
	public $arenas = array();
	public $currentLevel = "";
				...
				if($player->isOp())
				{
                                       ...
                                       
					{
						if($args[0]=="create")
						{
							...
							{
							                                             $player->sendMessage($this->prefix . "SkyWars Commands!");
                                             $player->sendMessage($this->prefix . "/sw create [world] Creates an arena in the specified world!");	
							}
						}
						else
						{
		...
		foreach($tiles as $t) {
			if($t instanceof Sign) {	
				...
				if($text[3]==$this->prefix)
				{
					...
					if($config->get($text[2] . "PlayTime")!=800)
					{
